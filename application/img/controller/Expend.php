...
class Expend extends Com{
	...
	public  function expendList($data){
        ...
        if(isset($data['start_time']) && isset($data['end_time'])){
            ...
            $end=$data['end_time'];
            $total=Db::table('expend_total')->where('add_time','between time',[$start,$end])->count();
            if($offset == 0){
                $record=Db::table('expend_total')->where('add_time','between time',[$start,$end])->order('add_time','desc')->limit($pagesize)->select();
                return renderJson('1','',['record'=>$record,'total'=>$total]);
            }
            $temple=Db::table('expend_total')->where('add_time','between time',[$start,$end])->order('add_time','desc')->limit($offset)->select();
            $tid=array_pop($temple);
            $record=Db::table('expend_total')->where('add_time','between time',[$start,$end])->where('id','<=',$tid['id'])->order('add_time','desc')->limit($pagesize)->select();
    ...
    public  function   exportList($data){
        ...
        if(isset($data['start_time']) && isset($data['end_time'])){
            ...
            $end=$data['end_time'];
            $total=Db::table('expend_total')->where('add_time','between time',[$start,$end])->count();
            $record=Db::table('expend_total')->where('add_time','between time',[$start,$end])->order('add_time','desc')->select();
            //写入日志
            $model->saveRecord($this->mid,$this->mname,$this->path,json_encode($param),json_encode(['code'=>'1','message'=>'','data'=>['record'=>$record,'total'=>$total]]));
            return renderJson('1','',['record'=>$record,'total'=>$total]);
        }
