...
class Stock extends Com{
	...
	public  function stockList($data){
        ...
        // $model->saveRecord($this->mid,$this->mname,$this->path,json_encode($param),json_encode(['code'=>'10001','message'=>'参数不合法']),$url);
            ...
            $record=Db::table('stock_total')->where('add_time','between ',[$start,$end])->order('add_time','desc')->limit($offset,$pagesize)->select();
        ...
        $record=Db::table('stock_total')->where('id','<=',$tid['id'])->order('add_time','desc')->limit($offset,$pagesize)->select();
        // //写入日志
        // $model->saveRecord($this->mid,$this->mname,$this->path,json_encode($param),json_encode(['code'=>'1','message'=>'']),$url);
        return renderJson('1','',['record'=>$record,'total'=>$total]);
	}
