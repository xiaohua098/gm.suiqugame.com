   ...
   public  function dailyList($data){
    $url='http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
        ...
        // $model->saveRecord($this->mid,$this->mname,$this->path,json_encode($param),json_encode(['code'=>'10001','message'=>'参数不合法']),$url);
            ...
            $record=Db::table('agent_daily')->where('uid',$uid)->where('add_time','between ',[$start,$end])->order('add_time','desc')->limit($offset,$pagesize)->select();
        ...
        $record=Db::table('agent_daily')->where('uid',$uid)->order('add_time','desc')->limit($offset,$pagesize)->select();
        // //写入日志
        // $model->saveRecord($this->mid,$this->mname,$this->path,json_encode($param),json_encode(['code'=>'1','message'=>'']),$url);
        return renderJson('1','',['record'=>$record,'total'=>$total]);
    }
